...
wb <- loadWorkbook("plant census 2014-2016.xlsx")
...
dfcurrent <- makecleandf(dataforyear=2016, prelimseedlingsforyear=NULL, includepreviousyearnotes=FALSE)
# export file
write.csv(dfcurrent, file='census 2016 clean.csv',row.names=F,na="")


# make clean dataframe
dfcurrent <- makecleandf(dataforyear=2015, prelimseedlingsforyear=NULL, includepreviousyearnotes=FALSE)
# export file
write.csv(dfcurrent, file='census 2015 clean.csv',row.names=F,na="")

dfly <- makecleandf(dataforyear=2014, prelimseedlingsforyear=NULL, includepreviousyearnotes=FALSE)
# export file
write.csv(dfly, file='census 2014 clean.csv',row.names=F,na="")
...
write.csv(abundancescurrent,'baldy abundances current 2016 bblonder.csv')
...
drawplot <- function(data.census, plotid, dataforyear, prelimseedlingsforyear,xlim=c(-10,210),ylim=c(-10,210), specieshighlight=NA, title="")
{
	...
	if (nrow(plotdata) > 0)
	{
		for (i in 1:nrow(plotdata))
		{
			...
			{
				points(plotdata$X..cm.[i],plotdata$Y..cm.[i],col= whichcolor_text,pch=0,cex=0.3,lwd=0.5)
			}
...
pdf(width=5,height=5,file='census map 2016.pdf')
par(mar=c(2,2,1,1))
par(cex.axis=0.5)
for (i in 1:50)
{
	drawplot(data.census, i, 2016,NULL)
}
dev.off()



pdf(width=5,height=5,file='census map 2016 zoomed.pdf')
par(mar=c(2,2,1,1))
par(cex.axis=0.5)
for (i in 1:50)
{
	drawplot(data.census, i, 2016,NULL,xlim=c(-10,110),ylim=c(-10,110),title="Lower left")
	drawplot(data.census, i, 2016,NULL,xlim=c(90,210),ylim=c(-10,110),title="Lower right")
	drawplot(data.census, i, 2016,NULL,xlim=c(-10,110),ylim=c(90,210),title="Upper left")
	drawplot(data.census, i, 2016,NULL,xlim=c(90,210),ylim=c(90,210),title="Upper right")
}
dev.off()
...
{
	pdf(width=5,height=5,file=sprintf('census map 2016 %s.pdf',as.character(taxon)))
	par(mar=c(2,2,1,1))
	par(cex.axis=0.5)
	for (i in 1:50)
	{
		drawplot(data.census, i, 2016,NULL,specieshighlight=as.character(taxon),title=as.character(taxon))
	}
	dev.off()
}
